...
describe("", () => {
  ...
  const lidoAdapter = VAULTS.LIDO.ADAPTER
  ...
  const confidenceInterval = ethers.utils.parseEther(".000001") // 1/10M of an ether 
  const FEE_NUM = 95;
  const FEE_DENOM = 100;
    ...
    if (!(await resonateHelper.isQueueEmpty(_pool, false))) {
      for (let x = queueMarkers.consumerHead; x.lt(queueMarkers.consumerTail); x = x.add(1)) {
          ...
          for(let y = BigNumber.from(0); y.lt(packetsRemaining); y = y.add(1)) {
            ...
            await resonate.connect(whaleSigners[6]).submitProducer(_pool, ethers.utils.parseEther(".05"), false)
  ...
  async function fastForwardAndAppreciate(months: number) {
    ...
    let lidoWhale = ethers.provider.getSigner("0x41318419cfa25396b47a94896ffa2c77c6434040");
    await setupImpersonator("0x41318419cfa25396b47a94896ffa2c77c6434040");

    let preBal = await usdcCon.balanceOf(mockVault.address);

    console.log(`Balance Before: ${preBal}`)

    await usdcCon.connect(lidoWhale).transfer(mockVault.address, ethers.utils.parseEther("10"))
  ...
  before(async () => {
    return new Promise(async resolve => {
      ...
      usdcCon = new ethers.Contract(steth, abi, owner) as ERC20;
      ...
      for (const whale of whales) {
        ...
        await usdcCon.connect(lidoWhale).transfer(whale, ethers.utils.parseEther("600"))
      }
  ...
  describe('Creating positions and then cancelling them in both pools', async () => {
    ...
    it("should create a consumer position within both of those pools, and then cancel both", async () => {
      let depositAmount = packetSize.add(2)
  ...
  describe('Should match a producer to an existing consumer with partial activation of counter-party', async () => {
    ...
    let consumerDepositAmount = packetSize.add(2)
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      let appreciationAmount = ethers.utils.parseEther("10")
  ...
  describe("Should test the sandwich bot ability to withdraw and deposit shares again with no problems", async () => {
    ...
    step("deposit back into the pool", async () => {
      ...
      expect(await usdcCon.balanceOf(pool0_wallet)).to.be.closeTo(BigNumber.from(0), 5);
    })
  ...
  let producerDepositAmount = producerPacketSize.mul(2)
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      console.log(`Price per one share: ${await mockVault.convertToAssets(ethers.utils.parseEther("1"))}`)
      console.log(`Pool can Claim: ${await mockVault.previewRedeem(poolt0_bal)}`)

      // console.log(`Num Shares: ${numShares.toString()}`)
      // console.log(`Total Shares: ${totalSupply.toString()}`)
  ...
  describe("Should match a producer to an existing consumer with multi counter-party activation (overflow)", async () => {
    ...
    step('producer fills consumer order for type-0 pool', async () => {
      ...
      await resonate.connect(whaleSigners[0]).submitConsumer(pool_t1, consumerDepositAmount, false); //consumer order 1k Tokens
      await resonate.connect(whaleSigners[1]).submitConsumer(pool_t1, consumerDepositAmount, false); //consumer order 1k Tokens
  ...
  describe("Should match a consumer to an existing producer with multi counter-party activation (overflow)", async () => {
    ...
    step('fast forward 6 more months and simulate vault appreciation', async () => {
      ...
      let expectedInterest = ethers.utils.parseEther((percentInterestOwed * numTokensAppreciation / precision).toFixed(18).toString())
      // let expectedInterest = BigNumber.from(Math.floor(percentInterestOwed * numTokensAppreciation / precision))
  ...
  describe('Should match a producer to an existing consumer with full activation for multiple packets on each side', async () => {
    ...
    let consumerDepositAmount = packetSize.mul(5).add(5)
    let producerDepositAmount = producerPacketSize.mul(5)
  ...
  describe("Should Test Exchange Rate Fluctuations while in Queue", async () => {
    ...
    let consumerDepositAmount = ethers.utils.parseEther("3").add(1) //3 StETH
    ...
    step("Create a cross-asset pools for both types using frax to payout for interest on StETH", async () => {
      ...
      await mockOracleDispatch.setPrice(frax, ethers.constants.WeiPerEther);
      await mockOracleDispatch.setPrice(usdcCon.address, ethers.utils.parseUnits('1800', 18));

      console.log("Successfully created oracles");

      pool_t0 = await resonate.callStatic.createPool(fraxCon.address, mockVault.address, rate, 0, YEAR, packetSize, "ca_pool_t0");
      await resonate.createPool(fraxCon.address, mockVault.address, rate, 0, YEAR, packetSize, "ca_pool_t0");
  
      pool_t1 = await resonate.callStatic.createPool(fraxCon.address, mockVault.address, rate, additionalRate, 0, packetSize, "ca_pool_t1");
      await resonate.createPool(fraxCon.address, mockVault.address, rate, additionalRate, 0, packetSize, "ca_pool_t1");
    ...
    step("Should lower exchange rate and check that consumer position cannot be completely filled", async () => {
      ...
      await resonate.connect(whaleSigners[7]).submitConsumer(pool_t1, consumerDepositAmount, false); //Consumer order 3 wETH
  ...
  describe("Should match producer (frax) to a consumer (stETH) for a cross-asset pool", async () => {
    if (skip) {
      return;
    }

    let consumerDepositAmount = packetSize.add(2) //TODO: Change Possibly
    let producerDepositAmount = ethers.utils.parseEther('90') //TODO: Change Possibly
  ...
  async function main() {
    ...
    const RariVault = await mockVaultFactory.deploy(steth)
