...
describe("", () => {
  ...
  let mockVault: MasterChefAdapter;
  ...
  const daiethSLP = TOKEN.daieth_slp
  const dai = TOKEN.dai
  const sushiswap = PROTOCOL.SUSHISWAP.ROUTER[1]
  const masterchef = PROTOCOL.MASTERCHEF.V1
  ...
  const packetSize = ethers.utils.parseEther("10")
  const producerPacketSize = ethers.utils.parseEther(".5")
  ...
  async function fastForwardAndAppreciate(months: number) {
    ...
    let sushiWhale = ethers.provider.getSigner("0xf977814e90da44bfa03b6295a0616a897441acec");
    await setupImpersonator("0xf977814e90da44bfa03b6295a0616a897441acec");

    let preBal = await sushiCon.balanceOf(mockVault.address);

    console.log(`Balance Before: ${preBal}`)

    await sushiCon.connect(sushiWhale).transfer(mockVault.address, ethers.utils.parseEther("10000"))

    let afterBal = await sushiCon.balanceOf(mockVault.address);
  ...
  before(async () => {
    return new Promise(async resolve => {
      ...
      usdcCon = new ethers.Contract(daiethSLP, abi, owner) as ERC20;
      fraxCon = new ethers.Contract(frax, abi, owner) as ERC20;

      whaleSigners = [];
      randomSigners = [];

      let lidoWhale = ethers.provider.getSigner("0x647481c033a4a2e816175ce115a0804adf793891");
      await setupImpersonator("0x647481c033a4a2e816175ce115a0804adf793891");
      ...
      for (const whale of whales) {
        ...
        await usdcCon.connect(lidoWhale).transfer(whale, ethers.utils.parseEther("110"))
      }
  ...
  describe('Should match a producer to an existing consumer with partial activation of counter-party', async () => {
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      let numTokens = await mockVault.valueRewardTokens();
  ...
  describe("Should Test Exchange Rate Fluctuations while in Queue", async () => {
    ...
    step("Onboard new oracle for cross-asset pool", async () => {
      ...
      // await resonate.registerOracle(usdcCon.address, frax, mockOracleDispatch.address)
      // expect(await resonate.oracleDispatch(usdcCon.address, frax)).to.eq(mockOracleDispatch.address)
      // await mockOracleDispatch.setPrice(usdcCon.address, frax, ethers.utils.parseEther("1800"));
  ...
  async function main() {
    ...
    const mockVaultFactory = await ethers.getContractFactory("MasterChefAdapter")
    // const RariVault = mockVaultFactory.attach(lidoAdapter)
    const RariVault = await mockVaultFactory.deploy(daiethSLP, masterChefpool, [sushi, weth], [sushi, weth, dai], sushiswap, masterchef, sushi, WETH)
    ...
    console.log(separator);
    console.log(`\tDeploying Sushiswap Router`)
    const masterchefFactory = await ethers.getContractAt("IMasterChef", masterchef) as IMasterChef
    ...
    console.log(`Sandwich Bot Proxy: ${masterChefCon.address}`)
