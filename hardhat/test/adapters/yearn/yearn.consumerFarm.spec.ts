...
describe("", () => {
  ...
  let mockVault: YearnWrapper;
  let mockWethVault: YearnWrapper;
  let sushiVault: YearnWrapper;
  let usdtVault: YearnWrapper_usdt;
  ...
  const yvusdc = TOKEN.yvUSDC;
  const yvusdt = TOKEN.yvUSDT;
  const yvweth = TOKEN.yvWETH;
  const yvsushi = TOKEN.yvSUSHI;
  ...
  async function fastForwardAndAppreciate(months: number) {
    ...
    await usdcCon.connect(usdcWhale).transfer(yvUSDCCon.address, ethers.utils.parseUnits("10000000", 6)) //send 10M to the vault
  ...
  before(async () => {
    return new Promise(async resolve => {
      ...
      yvUSDCCon = new ethers.Contract(yvusdc, abi, owner) as ERC20;
      yvUSDTCon = new ethers.Contract(yvusdt, abi, owner) as ERC20;
      yvWETHCon = new ethers.Contract(yvweth, abi, owner) as ERC20;
      yvSushiCon = new ethers.Contract(yvsushi, abi, owner) as ERC20;
  ...
  it("should onboard an acceptable yield farm", async () => {
    console.log(`Vault Addr: ${yvusdc}`)
    await resonate.modifyVaultAdapter(yvusdc, mockVault.address)
    expect(await resonate.vaultAdapters(yvusdc)).to.eq(mockVault.address)

  });

  it("should create both types of pools", async () => {
    pool_t0 = await resonate.callStatic.createPool(usdcCon.address, yvusdc, rate, 0, YEAR, packetSize, "pool_t0");
    await resonate.createPool(usdcCon.address, yvusdc, rate, 0, YEAR, packetSize, "pool_t0");

    pool_t1 = await resonate.callStatic.createPool(usdcCon.address, yvusdc, rate, additionalRate, 0, packetSize, "pool_t1");
    await resonate.createPool(usdcCon.address, yvusdc, rate, additionalRate, 0, packetSize, "pool_t1");
  ...
  describe('Creating positions and then cancelling them in both pools', async () => {
    ...
    it("should create a consumer position within both of those pools, and then cancel both", async () => {
      ...
      console.log(`Preview Deposit: ${await mockVault.previewDeposit(depositAmount)}`)
      ...
      console.log(`Shares per 1000 tokens: ${await mockVault.convertToShares(packetSize)}`)
  ...
  describe('Should match a producer to an existing consumer with partial activation of counter-party', async () => {
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      let appreciationAmount = 30000000
      ...
      let totalSupply =  (await yvUSDCCon.totalSupply()).toNumber()
      let adapterSupply =  (await yvUSDCCon.balanceOf(mockVault.address)).toNumber()

      let percentOwnership = adapterSupply / totalSupply;
      let adapterInterest = percentOwnership * appreciationAmount
      ...
      let expectedInterest = ethers.utils.parseUnits((adapterInterest / 4).toFixed(6).toString(), 6)
      ...
      //Edge case test where someone deposits into the adapter outside of Resonate.
      await usdcCon.connect(whaleSigners[0]).approve(mockVault.address, ethers.utils.parseUnits("10000", 6))
      await mockVault.connect(whaleSigners[0]).deposit(ethers.utils.parseUnits("250", 6), whaleSigners[0]._address);
    })
    ...
    step('withdraw the interestFNFT interest from pool-0 while principal still exists', async () => {
      ...
      expect(afterBal).to.be.closeTo(preBal.add(val.interestAfterFee),15)
  ...
  describe("Should test the sandwich bot ability to withdraw and deposit shares again with no problems", async () => {
    ...
    step("withdraw from pool", async () => {
      ...
      let pool0_wallet_shares_after = await mockVault.balanceOf(pool0_wallet)
      expect(pool0_wallet_shares_after).to.be.closeTo(BigNumber.from(0), 10);
    })
    ...
    step("deposit back into the pool", async () => {
      ...
      expect(pool0_wallet_shares_after).to.be.closeTo(pool0_wallet_shares_before, 2)
      expect(await usdcCon.balanceOf(pool0_wallet)).to.eq(0);
    })
  })
  ...
  //producer completely fills consumer order and has extra parts of their order left over
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      let totalSupplyyvUSDC = (await yvUSDCCon.totalSupply()).toNumber()
      let percentAdapterOwed = (await yvUSDCCon.balanceOf(mockVault.address)).toNumber()

      let numTokensAppreciation = (10000000*2) * (percentAdapterOwed / totalSupplyyvUSDC)
  ...
  describe("Should match a consumer to an existing producer with multi counter-party activation (overflow)", async () => {
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      await fastForwardAndAppreciate(6)
      await fastForwardAndAppreciate(6)
    ...
    step('fast forward 6 more months and simulate vault appreciation', async () => {
      ...
      let expectedInterest = ethers.utils.parseUnits((percentInterestOwed * numTokensAppreciation / precision).toFixed(6).toString(), 6)
//       let expectedInterest = ethers.utils.parseEther((percentInterestOwed * numTokensAppreciation).toFixed(18).toString())
  ...
  describe('Should match a producer to an existing consumer with full activation for multiple packets on each side', async () => {
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      let numTokensAppreciation = (10000000*3) * (percentAdapterOwed / totalSupplyyvUSDC)
  ...
  describe("Should match producer (USDC) to a consumer (WETH) for a cross-asset pool", async () => {
    ...
    step("Create a cross-asset pools for both types using USDC to payout for interest on wETH", async () => {
      ...
      // await resonate.registerOracle(sushi, fraxCon.address, mockOracleDispatch.address)
    ...
    step("match consumer to provider for type-0 pool", async () => {
      ...
      await resonate.connect(whaleSigners[7]).submitConsumer(pool_t0, consumerDepositAmount, true); //consumer order 1 wETH
      await resonate.connect(whaleSigners[1]).submitProducer(pool_t0, producerDepositAmount, false); //Producer order 90 USDC
    ...
    step("match consumer to provider for type-1 pool", async () => {
      ...
      await resonate.connect(whaleSigners[7]).submitConsumer(pool_t1, consumerDepositAmount, true); //consumer order 1 wETH
  ...
  describe("Should match producer (FRAX) to a consumer (SUSHI) for a cross-asset pool", async () => {
    ...
    step("Onboard new oracle for cross-asset pool", async () => {
      ...
      // not sure what the price of sushi is supposed to be set to - inquire with Rob
      await mockOracleDispatch.setPrice(sushi, ethers.constants.WeiPerEther); //Exchange rate 1 SUSHI = 1.5 Frax
      await mockOracleDispatch.setPrice(frax, exchangeRate); //Exchange rate 1 SUSHI = 1.5 Frax
    ...
    step("match consumer to provider for type-0 pool", async () => {
      ...
      await resonate.connect(whaleSigners[5]).submitConsumer(pool_t0, consumerDepositAmount, true); //consumer order 100 SUSHI
      ...
      expect(afterBalSushiConsumer).to.be.closeTo(preBalSushiConsumer.sub(consumerDepositAmount), wethConfidenceInterval);
    ...
    step("match consumer to provider for type-1 pool", async () => {
      ...
      await resonate.connect(whaleSigners[5]).submitConsumer(pool_t1, consumerDepositAmount, true); //consumer order 1 wETH
    ...
    step("fast forward 1 year to withdrawal and appreciate", async () => {
      ...
      let preBal = await sushiCon.balanceOf(yvSushiCon.address);
      console.log(`Pre Bal: ${preBal}`)

      await sushiCon.connect(sushiWhale).transfer(yvSushiCon.address, ethers.utils.parseEther("3000000")) //send 1000 SUSHI to the vault
      let afterBal = await sushiCon.balanceOf(yvSushiCon.address);
  ...
  describe("Testing regular functionality with USDT that has non-standard implementation", async () => {
    ...
    step("Match parties for type-0 pool", async () => {
      ...
      await resonate.connect(whaleSigners[3]).submitConsumer(pool_t0, consumerDepositAmount, true); //consumer order 20k Tokens
      await resonate.connect(whaleSigners[0]).submitProducer(pool_t0, producerDepositAmount, false); //Producer order 10k Tokens
    ...
    step("Match parties for type-1 pool", async () => {
      ...
      console.log(`Creating positions for type-0 pool`)
      principalIdP2 = await fnftHandler.getNextId()
      await resonate.connect(whaleSigners[3]).submitConsumer(pool_t1, consumerDepositAmount, true); //consumer order 20k Tokens
    ...
    step('fast forward 1 year and simulate vault appreciation', async () => {
      ...
      const whale = ethers.provider.getSigner("0x5041ed759Dd4aFc3a72b8192C143F72f4724081A");
      setupImpersonator("0x5041ed759Dd4aFc3a72b8192C143F72f4724081A");

      //TODO - Appreciate Vault
      // console.log(`Balance of whaleSigner: ${await usdtCon.balanceOf(whaleSigners[5]._address)}`)
      await usdtCon.connect(whale).transfer(yvUSDTCon.address, ethers.utils.parseUnits("5000000", 6));
  ...
  describe("Should match producer (FRAX) to a consumer (USDC) for a cross-asset pool", async () => {
    ...
    step("Onboard new oracle for cross-asset pool", async () => {
      ...
      await mockOracleDispatch.setPrice(frax, ethers.constants.WeiPerEther); //Exchange rate 1 FRAX = 1 USDC
      await mockOracleDispatch.setPrice(usdcCon.address, ethers.constants.WeiPerEther); //Exchange rate 1 FRAX = 1 USDC
    });
  ...
  async function main() {
    ...
    let factory = await ethers.getContractFactory("YearnWrapper")
    const RariVault = await factory.deploy(yvusdc)
    await Resonate.modifyVaultAdapter(yvusdc, RariVault.address)
    console.log(`Vault Adapter Address: ${RariVault.address}`)
    
    console.log("\tDeploying Mock WETH Rari Vault")
    factory = await ethers.getContractFactory("YearnWrapper")
    const wethRariVault = await factory.deploy(yvweth)
    await Resonate.modifyVaultAdapter(yvweth, wethRariVault.address)

    console.log(separator);
    console.log("\tDeploying Mock USDT Rari Vault")
    let USDTfactory = await ethers.getContractFactory("YearnWrapper_usdt")
    const TetherVault = await USDTfactory.deploy(yvusdt) as YearnWrapper_usdt
    await Resonate.modifyVaultAdapter(yvusdt, TetherVault.address)

    console.log(separator);
    console.log("\tDeploying Mock Sushi Vault")
    factory = await ethers.getContractFactory("YearnWrapper")
    const mockSushiVault = await factory.deploy(yvsushi)
    await Resonate.modifyVaultAdapter(yvsushi, mockSushiVault.address)
