...
describe("", () => {
  ...
  describe("Should match a producer to an existing consumer with multi counter-party activation (overflow)", async () => {
    ...
    step('producer fills consumer order for type-0 pool', async () => {
      ...
      await resonate.connect(whaleSigners[0]).submitConsumer(pool_t1, consumerDepositAmount, true); //consumer order 1k Tokens
      await resonate.connect(whaleSigners[1]).submitConsumer(pool_t1, consumerDepositAmount, true); //consumer order 1k Tokens
  ...
  describe("Should Test Exchange Rate Fluctuations while in Queue", async () => {
    ...
    step("Should lower exchange rate and check that consumer position cannot be completely filled", async () => {
      ...
      await resonate.connect(whaleSigners[7]).submitConsumer(pool_t1, consumerDepositAmount, true); //Consumer order 3 wETH
