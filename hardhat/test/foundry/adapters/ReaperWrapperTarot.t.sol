...
contract ReaperWrapperTarotTest is AdapterTest {

    ReaperWrapper ReaperAdapter;
    address cryptFantom = 0xb4bb795B165FB0fBF11598a3c6E3D011EF5d9dF8;
    ...
    */
    ...
    function testWithdraw(uint amount, uint withdrawAmount) public virtual override {
        ReaperStrat strat = ReaperStrat(ReaperAPI(cryptFantom).strategy());
        vm.prank(strat.getRoleMember(strat.DEFAULT_ADMIN_ROLE(), 0));
        strat.setMinWantToDepositOrWithdraw(0);
        ...
        assertLe(preview, adapter.balanceOf(alice), "Alice does not have enough shares to withdraw");
    ...
    function testRoundTrip_deposit_withdraw(uint amount) public virtual override {
        ...
        assertLe(previewWithdraw, adapter.balanceOf(alice), "Alice does not have enough shares to withdraw");
