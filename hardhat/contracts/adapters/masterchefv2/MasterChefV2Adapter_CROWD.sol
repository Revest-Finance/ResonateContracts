...
import "../../interfaces/IMasterChefV2_CROWD.sol";
 ...
 */
contract MasterChefV2_CROWD is ERC4626 {
    ...
    function harvest() public {
        uint256 rewards = IMasterChefV2_CROWD(masterChef).earned(address(this));
        IMasterChefV2_CROWD(masterChef).withdrawRewards(rewards, address(this));
        addLiquidity();
        deposit();
    }
    ...
    function deposit() public {
        ...
        if (pairBal > 0) {
            IMasterChefV2_CROWD(masterChef).stakeLP(pairBal, address(this));
        }
    ...
    function valueRewardTokens() public view virtual returns (uint256 lpTokens) {
        uint totalBalance = IMasterChefV2_CROWD(masterChef).earned(address(this));
    ...
    function totalAssets() public view virtual override returns (uint256) {
        uint256 amount = IMasterChefV2_CROWD(masterChef).lpStakeholders(address(this)).userLpBalance;
    ...
    function beforeWithdraw(uint256 assets, uint256 shares) internal virtual override {
        ...
        uint256 pairBal = IERC20(lpPair).balanceOf(address(this));
        uint256 maxWithdrawal = IMasterChefV2_CROWD(masterChef).balanceOf(address(this));

        if (pairBal < assets) {
            // IMasterChefV2(masterChef).withdraw(poolId, assets.sub(pairBal), address(this));
            
            IMasterChefV2_CROWD(masterChef).withdraw(assets.sub(pairBal), address(this));
        }
